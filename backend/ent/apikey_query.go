...
func (_q *APIKeyQuery) QueryOauthManagedAPIKeys() *OAuthManagedAPIKeyQuery {
...
func (_q *APIKeyQuery) WithOauthManagedAPIKeys(opts ...func(*OAuthManagedAPIKeyQuery)) *APIKeyQuery {
	query := (&OAuthManagedAPIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withOauthManagedAPIKeys; query != nil {
		if err := _q.loadOauthManagedAPIKeys(ctx, query, nodes,
			func(n *APIKey) { n.Edges.OauthManagedAPIKeys = []*OAuthManagedAPIKey{} },
			func(n *APIKey, e *OAuthManagedAPIKey) {
...
func (_q *APIKeyQuery) loadOauthManagedAPIKeys(ctx context.Context, query *OAuthManagedAPIKeyQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *OAuthManagedAPIKey)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(oauthmanagedapikey.FieldAPIKeyID)
	}
	query.Where(predicate.OAuthManagedAPIKey(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.OauthManagedAPIKeysColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
